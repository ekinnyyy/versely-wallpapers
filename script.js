...
function cardTemplate(wallpaper, cardIndex) {
  ...
  const creditUrl = safeUrl(wallpaper.credit_url || "");

  const slides = images.map((image, imageIndex) => {
    const src = safeUrl(image);
    return `
      <div class="carousel-slide">
        <button class="image-button" type="button" data-full="${esc(src)}" aria-label="Preview ${title}, version ${imageIndex + 1}">
          <img class="wallpaper-image" src="${esc(src)}" alt="${title} — version ${imageIndex + 1}" loading="lazy">
        </button>
      </div>`;
  }).join("");
  ...
  const dots = hasMultiple ? `
    <div class="carousel-dots" aria-label="Wallpaper versions">
      ${images.map((_, i) => `<button type="button" class="carousel-dot ${i === 0 ? "active" : ""}" data-index="${i}" aria-label="Show version ${i + 1}"></button>`).join("")}
    </div>` : "";
  ...
  const counter = hasMultiple ? `<span class="variant-counter"><span class="counter-current">1</span>/${images.length}</span>` : "";

  const credit = creditName ? `
    <p class="credit">Credit: ${creditUrl ? `<a href="${esc(creditUrl)}" target="_blank" rel="noopener noreferrer">${creditName} ↗</a>` : creditName}</p>` : "";
    ...
    <article class="wallpaper-card" data-card-index="${cardIndex}">
      ...
      <div class="card-body">
        ...
        <div class="card-actions">
          ${hasMultiple ? `<span class="current-variant">Version <span class="version-number">1</span></span>` : ""}
          <a class="download-button" href="${esc(safeUrl(images[0]))}" download>Download HD</a>
        </div>
      </div>
    </article>`;
}
...
function setCarouselIndex(carousel, newIndex) {
  ...
  if (versionNumber) versionNumber.textContent = String(index + 1);

  const activeImage = carousel.querySelectorAll(".image-button")[index]?.dataset.full;
  const downloadButton = card.querySelector(".download-button");
  if (activeImage && downloadButton) downloadButton.href = activeImage;
}

function initializeCarousel(carousel) {
  carousel.querySelector(".prev")?.addEventListener("click", () => {
    setCarouselIndex(carousel, Number(carousel.dataset.index) - 1);
  });
  ...
  carousel.querySelectorAll(".carousel-dot").forEach((dot) => {
    dot.addEventListener("click", () => setCarouselIndex(carousel, Number(dot.dataset.index)));
  });
  ...
  carousel.addEventListener("touchend", (event) => {
    ...
    if (Math.abs(difference) > 40) {
      ...
      setCarouselIndex(carousel, difference < 0 ? current + 1 : current - 1);
    }
    startX = null;
  }, { passive: true });
}
...
fetch("/data/wallpapers.json", { cache: "no-store" })
  .then((response) => {
    if (!response.ok) throw new Error("Could not load wallpapers.json");
    return response.json();
  })
  .then((data) => {
    const items = (data.wallpapers || []).slice().reverse();
    grid.innerHTML = items.map((wallpaper, index) => cardTemplate(wallpaper, index)).join("");
  ...
  .catch((error) => {
    console.error(error);
    grid.innerHTML = `<p style="color:#756f67">Wallpapers could not be loaded.</p>`;
  });
